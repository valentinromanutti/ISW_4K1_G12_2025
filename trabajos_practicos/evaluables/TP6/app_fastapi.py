...
try:
    ...
    print("¡ADVERTENCIA! No se pudo importar 'inscripcion_actividad'. Asegúrate de que '\src\' esté en la ruta.")
    raise

# --- MODELOS PYDANTIC ---

class PersonaInscripcion(BaseModel):
    """Define la estructura de cada persona en la lista. Se asume que la función importada
       espera un diccionario con estos campos."""
    dni: int
    nombre: str
    edad: int
...
@app.post("/inscribir", status_code=201, response_model=dict, tags=["Inscripciones"])
def handle_inscripcion(request_data: InscripcionRequest):
    """
    Endpoint POST para registrar una o más personas a una actividad, llamando a la función externa.
    """
    ...
    try:
        # La función inscribir_actividad se encarga de toda la validación y la DB.
        ...
        # Respuesta exitosa
        return {"mensaje": "Inscripción realizada con éxito.", "datos": resultado}

    except ValueError as e:
        # Errores de validación de negocio (capturados de la función importada)
        raise HTTPException(status_code=400, detail=str(e))

    except Exception as e:
        # Errores internos o de base de datos
        print(f"Error interno: {e}")
        # Se puede agregar un manejo específico para FileNotFoundError si la DB falla.
        if "FileNotFoundError" in str(e):
             raise HTTPException(status_code=500, detail="Error de configuración: Base de datos no encontrada.")

        raise HTTPException(status_code=500, detail="Error interno del servidor al procesar la inscripción.")
